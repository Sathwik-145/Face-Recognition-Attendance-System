...
def postAttendance(name):
    tDate = date.today()
    try:
        os.makedirs("attendanceSheet", exist_ok=True)
        with open(f'attendanceSheet/{tDate}.csv', "x") as f:
            f.writelines('Name,InTime,OutTime\n')
    except FileExistsError:
        pass

    with open(f"attendanceSheet/{tDate}.csv", "r+") as f:
        currentFile = f'attendanceSheet/{tDate}.csv'
        namesList = []
        inTimeList = []
        outTimeList = []
...
style.configure("Treeview", 
    ...
    foreground="black",  # Text color (black for attendees)
    font=("Helvetica", 16), 
    rowheight=30, 
    borderwidth=1,  # Thin border
    relief="solid",  # Adds solid border
    highlightthickness=0,  # Removes highlight border
    lightcolor="black",  # Vertical lines
    darkcolor="black",   # Horizontal lines
)

style.map("Treeview", 
    background=[("selected", "lightblue")],  # Row selection background color
    foreground=[("selected", "black")]  # Row selection text color (opposite)
)
...
style.configure("Treeview.Heading", 
    ...
    relief="solid",  # Adds solid border to the header
    borderwidth=1
)

# Ensure horizontal and vertical lines are black
style.configure("Treeview", 
    lightcolor="black",  # Vertical lines
    darkcolor="black",   # Horizontal lines
)
